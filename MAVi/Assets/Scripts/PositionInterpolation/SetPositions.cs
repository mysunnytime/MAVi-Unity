...
public class SetPositions : MonoBehaviour
{
    ...
    private void Update()
    {
...

        {
            ...
            Camera.main.transform.position = oneMinusT * oneMinusT * oneMinusT * oneMinusT * Position1 +
                    4f * oneMinusT * oneMinusT * oneMinusT * t * Position2 +
                    6f * oneMinusT * oneMinusT * t * t * Position3 +
                    4f * oneMinusT * t * t * t * Position4 +
                    t * t * t * t * Position5;
    ...
    /*  timeVar = timeVar + Time.deltaTime;
      while (timeVar < Time.time + overTime) 

      {
          t = Mathf.Clamp01((timeVar)/overTime);
        //  t = BezierTime;
          float oneMinusT = 1f - t;
          Camera.main.transform.position = oneMinusT * oneMinusT * oneMinusT * oneMinusT * Position1 +
                  4f * oneMinusT * oneMinusT * oneMinusT * t * Position2 +
     ...
     Camera.main.transform.position = Position5;
     return;
}*/
    /*void Update()
      {
    ...
    /*
        ...
        {
            return transform.TransformPoint(Bezier.GetPoint(points[0], points[1], points[2], points[3], points[4], t));
        }
